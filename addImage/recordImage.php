<?php
session_name("project_arloing_ducroquetz");
session_start();
...
if (! (isset($_SESSION["ident"]) && ($_SESSION["ident"]->login == "admin1" || $_SESSION["ident"]->login == "admin7"))) {
	exit();
}
...
class Image {
    ...
    public function saveToDataBase(){ /* renvoyer true uniquement en cas de succès */
        ...
        $stmt = $connexion->prepare('
            INSERT INTO PicOne.photos ("url", "mime_type", "licence", "size", "thumbnail", "author", "url_author", "title", "categories", "keywords") 
            VALUES (:url, :mime_type, :licence, :size, :thumbnail, :author, :url_author, :title, :categories, :keywords)');
				$stmt->bindValue(":url", $this->url);
				$stmt->bindValue(":mime_type", $this->mime_type);
				$stmt->bindValue(":licence", $this->licence);
				$stmt->bindValue(":size", $this->size);
				$stmt->bindValue(":thumbnail", $this->thumbnail);
				$stmt->bindValue(":author", $this->author);
				$stmt->bindValue(":url_author", $this->url_author);
				$stmt->bindValue(":title", $this->title);
				$stmt->bindValue(":categories", implode(' ', $this->categories));
